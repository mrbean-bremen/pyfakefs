...
class FakeOsModuleTest(FakeOsModuleTestBase):
    ...
    def test_circular_readlink_with_trailing_separator_macos(self):
        ...
        file_path = self.make_path("foo")
        self.os.symlink(file_path, file_path)
        # the behavior has changed in newer macOS versions, except both
        # no exception and recursion exception
        try:
            self.os.readlink(file_path + self.os.sep)
        except OSError as e:
            self.assertEqual(e.errno, errno.ELOOP)
    ...
    def test_readlink_circular_link_with_trailing_sep_posix(self):
        self.check_posix_only()
